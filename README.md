...
v1.7.0 的完整日志页面支持整份日志搜索、正则表达式、上一个/下一个匹配、ERROR 跳转、精确行号、目标行高亮、复制错误块、字号调节、自动换行以及大文件分页读取。

搜索和 ERROR 索引会流式扫描日志，不会把整份大日志一次性加载进内存。


## 日志分析工作台

v1.8.0 新增整份日志 Tag / PID / 进程 / 包名 / 日志等级组合过滤、自定义过滤预设和按日志文件持久保存的书签。搜索、ERROR 和过滤结果都可以收藏位置，之后重新打开同一份历史日志仍可跳回对应行。


## 实时 Logcat 分析

v1.9.0 在录制过程中实时统计最近 60 秒日志等级、日志速率、ERROR 速率、Top Tag 和 PID / 进程变化；首页预览新增 D+/I+/W+/E+、Tag、PID、进程/包名组合过滤。实时过滤只影响屏幕预览，不会影响完整日志落盘。


## 记录会话

v2.0.0 引入会话化排错流程。每次开始 Logcat 记录都会自动创建一个会话，保存目标应用、记录模式、开始/结束时间、日志路径、日志大小和最终状态。

新的工作区首页统一提供实时日志、会话、历史日志、分析工作台和崩溃分析入口。
